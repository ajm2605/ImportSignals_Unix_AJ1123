function [NEX] = exportNCS(foldername,savelocation,tarlocation,tarNCS,extension,exMethod,exportNEX,datatype,idxmeth)
...
if ~isempty(tarlocation)
    [~,simplefolder]=fileparts(foldername);
    if tarNCS
        tarpath=fullfile(tarlocation, [simplefolder '_NCS.tar']);
        tar(tarpath,filelist);
        exfiles=[exfiles tarpath];
    end
    tar(fullfile(tarlocation, [simplefolder '.tar']),exfiles);
    if tarNCS
